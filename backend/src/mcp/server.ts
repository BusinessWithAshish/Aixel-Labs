...
export function createAixelIntelligenceMcpServer(): McpServer {
  ...
  server.registerTool(
    ...
    {
      description:
        "Transcribe a video or audio file into a plain transcript using Groq Whisper — no summarization, just the raw transcript. Resolves the source (local path or URL) server-side, extracts + normalizes its audio via ffmpeg (16kHz mono), then calls Groq's speech-to-text API.\n\n`mediaSource` accepts a local filesystem path (read directly off disk) or any publicly-reachable URL (S3, a CDN, Google Drive's direct-download URL, etc.). It does NOT accept raw file bytes — MCP tool calls can't carry large binary payloads.\n\n`format` controls the output shape: txt (plain text, default), json (full Groq verbose_json with segment timestamps), srt/vtt (caption files built from those segment timestamps — Groq has no native srt/vtt output, so these are generated here). Optional `language` (ISO-639-1) improves accuracy; optional `model` overrides the default whisper-large-v3-turbo.\n\nDo NOT use this for YouTube video URLs — use youtube_get_video_transcript_intelligence instead, which fetches YouTube's own caption track rather than re-transcribing audio. Downloaded/derived temp files are deleted after transcribing — nothing is persisted; a local-path source is never deleted.",
