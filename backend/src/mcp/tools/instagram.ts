...
import { fetchInstagramPopularSearch } from "../../api/instagram/advanced/popular/client";
import { IG_POPULAR_REQUEST_SCHEMA } from "../../api/instagram/advanced/popular/schemas";
...
const INSTAGRAM_OPS: Record<string, DomainOp> = {
  ...
  popular: {
    defaultLayer: MCP_LAYER.RAW,
    raw: {
      schema: IG_POPULAR_REQUEST_SCHEMA,
      run: fetchInstagramPopularSearch,
    },
  },
  account: {
...
- popular (raw only) — native IG /popular/{q}/ reels (Puppeteer). Fallback when both GSearch ops are thin. input: query, country?, maxReels?, enrichProfiles?
...
Discovery order: search_profiles → content_leads → popular.`;
