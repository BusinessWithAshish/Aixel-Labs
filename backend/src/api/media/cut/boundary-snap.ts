...
export function snapClipBoundaries(
  ...
  if (diarized && diarized.segments.length > 0) {
    ...
    if (containingStart && rawStart - containingStart.start <= MEDIA.BOUNDARY_SNAP_MAX_DISTANCE_SECONDS) {
    ...
    if (containingEnd && containingEnd.end - rawEnd <= MEDIA.BOUNDARY_SNAP_MAX_DISTANCE_SECONDS) {
      end = containingEnd.end;
      snapped = true;
    }
  }

  start = Math.max(0, start - MEDIA.CLIP_PADDING_SECONDS);
  end = end + MEDIA.CLIP_PADDING_SECONDS;
