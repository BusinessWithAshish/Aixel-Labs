...
import { MEDIA_CONDENSE, MEDIA_CONDENSE_DEFAULT_FILLER_WORDS } from "./constants";
...
export function findFillerRanges(
  ...
  const dictionary = new Set(
    (fillerWords ?? MEDIA_CONDENSE_DEFAULT_FILLER_WORDS).map(normalizeWord),
  );
  ...
  for (let i = 0; i < words.length; i += 1) {
    ...
    const guard = Math.min(
      MEDIA_CONDENSE.FILLER_INWARD_GUARD_SECONDS,
      (word.end - word.start) * MEDIA_CONDENSE.FILLER_GUARD_MAX_FRACTION,
    );
    ...
    if (end - start >= MEDIA_CONDENSE.MIN_REMOVAL_SECONDS) {
