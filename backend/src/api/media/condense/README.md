# Condense (`POST /media/condense`)

Takes a video, removes the dead air and the filler words, and gives back one
condensed video. No clip selection, no reframing, no LLM judgement — the whole
video comes back, just with the wasted time cut out of it.

This is **not** clip selection. `cut` takes chosen *moments* and lifts them
*out* of a source. `condense` keeps the whole video and removes the gaps
*inside* it. The two compose fine — condense first, then cut — but they are
separate operations on separate endpoints.

## Request — `POST /media/condense`
...
Schema: `schemas.ts` → `MEDIA_CONDENSE_REQUEST_SCHEMA`. Bounds/defaults:
`constants.ts` → `MEDIA_CONDENSE`.
...
`resolveMediaSource` in `../source.ts`.

## Response — `ALApiResponse<MEDIA_CONDENSE_RESPONSE>`

```ts
{
  videoPath: "/opt/aixel/storage/video-condense-output/condensed-....mp4",
...
way, condensing is a knob-tuning operation: re-running with a different
threshold shouldn't require re-fetching the video, so nothing about the input
is deleted beyond this call's own temp files.

## Deployment

HTTP `/media/condense` and the `media` MCP tool (`op=condense`) always
...
Same reasoning as the rest of the `media` module (see its README): Blob only existed to hand
...
storage limit besides. `condenseVideo` refuses to run at all on Vercel
...
1. `resolveMediaSource` (**reused from `../source.ts`**) — local path used
...
2. Concurrently, since neither depends on the other and both are full passes
   ...
   - `getWordTimestamps` (`client.ts`) — `normalizeToFlac` + `transcribeWithGroq`
     (**both reused from `../transcribe/`**) with `timestamp_granularities[]=word`.
...
6. `assembleKeepRanges` (`assemble.ts`) — one ffmpeg re-encode, written
   straight to `MEDIA_CONDENSE_OUTPUT_DIR`.
...
| `MEDIA_CONDENSE_VERBATIM_PROMPT` | "So, **um**, I was thinking, **uh**, that we should probably ship this today. **Er**, maybe tomorrow." | **3 of 3** |
...
video/condense/
  ...
  handler.ts       # thin: zod validate -> condenseVideo -> ALApiResponse
...
`video` MCP tool, `op=condense` (`backend/src/mcp/tools/video.ts`) wraps
`condenseVideo()` from `client.ts` directly — same function the HTTP handler
calls, no loopback — and reuses `MEDIA_CONDENSE_REQUEST_SCHEMA` as `input`.
