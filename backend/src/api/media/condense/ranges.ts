import { MEDIA_CONDENSE } from "./constants";
...
export function shrinkSilences(
  ...
  for (const { start, end } of silences) {
    ...
    if (shrunkEnd - shrunkStart >= MEDIA_CONDENSE.MIN_REMOVAL_SECONDS) {
...
export function mergeRanges(ranges: TIME_RANGE[]): TIME_RANGE[] {
  ...
  for (let i = 1; i < sorted.length; i += 1) {
    ...
    if (current.start - last.end <= MEDIA_CONDENSE.BRIDGE_GAP_SECONDS) {
...
export function capRanges(ranges: TIME_RANGE[]): {
  ...
  if (ranges.length <= MEDIA_CONDENSE.MAX_CUT_RANGES) {
    return { ranges, droppedCount: 0 };
  }

  const kept = [...ranges]
    .sort((a, b) => b.end - b.start - (a.end - a.start))
    .slice(0, MEDIA_CONDENSE.MAX_CUT_RANGES)
    .sort((a, b) => a.start - b.start);

  return { ranges: kept, droppedCount: ranges.length - MEDIA_CONDENSE.MAX_CUT_RANGES };
}
