...
function getApiKeyPool(): string[] {
  ...
  const keys = (raw ?? "")
    ...
    .filter(Boolean);
  if (keys.length === 0) {
    throw new Error(MEDIA_ERROR_MESSAGES.MISSING_API_KEY);
  }
  return keys;
}
...
function backoffDelayMs(attempt: number): number {
  return Math.min(
    MEDIA.GEMINI_RETRY_BASE_DELAY_MS * 2 ** attempt,
    MEDIA.GEMINI_RETRY_MAX_DELAY_MS,
  );
}
 ...
 * unit in diarize/ and the moments scorer, not individual HTTP calls.
 */
export async function withGeminiKeyPoolRetry<T>(
  ...
  for (const apiKey of keys) {
    for (let attempt = 0; attempt < MEDIA.GEMINI_MAX_ATTEMPTS_PER_KEY; attempt++) {
      try {
        ...
        const isLastAttemptOnThisKey = attempt === MEDIA.GEMINI_MAX_ATTEMPTS_PER_KEY - 1;
  ...
  throw new Error(
    `${MEDIA_ERROR_MESSAGES.GEMINI_KEY_POOL_EXHAUSTED} (${context}) — tried ${keys.length} key(s): ${lastMessage}`,
  );
}
...
export async function uploadFileToGemini(
  ...
  const startRes = await geminiFetch(
    `${MEDIA_GEMINI_BASE_URL}/upload/v1beta/files?key=${apiKey}`,
    {
      method: "POST",
      headers: {
  ...
  if (!uploadUrl) {
    ...
    throw new Error(
      `${MEDIA_ERROR_MESSAGES.GEMINI_UPLOAD_FAILED}: no upload URL (${startRes.status} ${body})`,
    );
  }
  ...
  if (fileInfo.error) {
    ...
    throw new GeminiApiError(
      `${MEDIA_ERROR_MESSAGES.GEMINI_UPLOAD_FAILED}: ${JSON.stringify(fileInfo.error)}`,
      info,
    );
  }
  if (!fileInfo.file) {
    throw new Error(`${MEDIA_ERROR_MESSAGES.GEMINI_UPLOAD_FAILED}: ${JSON.stringify(fileInfo)}`);
  }
...
export async function waitForGeminiFileActive(
  ...
  for (let attempt = 0; attempt < MEDIA.FILE_ACTIVE_POLL_MAX_ATTEMPTS; attempt++) {
    try {
      const res = await geminiFetch(
        `${MEDIA_GEMINI_BASE_URL}/v1beta/${fileName}?key=${apiKey}`,
      );
      ...
      if (info.state === "FAILED") {
        throw new Error(
          `${MEDIA_ERROR_MESSAGES.GEMINI_FILE_NOT_ACTIVE}: ${JSON.stringify(info)}`,
        );
      }
    ...
    await sleep(MEDIA.FILE_ACTIVE_POLL_INTERVAL_MS);
  }

  const suffix = lastPollError instanceof Error ? `: ${lastPollError.message}` : "";
  throw new Error(`${MEDIA_ERROR_MESSAGES.GEMINI_FILE_NOT_ACTIVE}${suffix}`);
}
...
export async function generateStructuredContent<T>(options: {
  ...
  const res = await geminiFetch(
    `${MEDIA_GEMINI_BASE_URL}/v1beta/models/${options.model}:generateContent?key=${options.apiKey}`,
    {
      method: "POST",
  ...
  if (genData.error) {
    ...
    throw new GeminiApiError(
      `${MEDIA_ERROR_MESSAGES.GEMINI_REQUEST_FAILED}: ${JSON.stringify(genData.error)}`,
      info,
    );
  }
  ...
  if (!text) {
    throw new Error(MEDIA_ERROR_MESSAGES.GEMINI_EMPTY_RESPONSE);
  }

  if (candidate?.finishReason && candidate.finishReason !== "STOP") {
    throw new Error(
      `${MEDIA_ERROR_MESSAGES.GEMINI_TRUNCATED_RESPONSE}: finishReason=${candidate.finishReason}, usage=${JSON.stringify(genData.usageMetadata ?? {})} — the response may look like valid JSON but is incomplete (e.g. a transcript that silently stops partway through a long episode). Not safe to use.`,
    );
  }
  ...
  if (options.zodValidator) {
    ...
    if (!result.success) {
      throw new Error(
        `${MEDIA_ERROR_MESSAGES.GEMINI_MALFORMED_RESPONSE}: ${result.error.message}`,
      );
    }
