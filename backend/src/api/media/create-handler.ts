...
>(options: {
  ...
  return async function videoHandler(req: Request, res: Response) {
    const parsed = schema.safeParse(req.body);

    if (!parsed.success) {
      res.status(400).json({
        success: false,
        error: MEDIA_ERROR_MESSAGES.INVALID_PARAMS,
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : MEDIA_ERROR_MESSAGES.GENERIC;
