/**
 * Diarization prompts — Gemini audio only. The YouTube-caption text-pass
 * prompt moved to `api/youtube/diarize/constants.ts` along with the rest of
 * that path. Limits shared with the rest of `media` (chunking, Gemini retry
 * policy) stay in the root `constants.ts`.
 */
 ...
 * backend/src/api/media/README.md) — keep this in sync with any prompt
 * change made during future testing.
 */
const MEDIA_DIARIZATION_SHARED_RULES = `- Produce a segment-level transcript: each segment is one continuous turn by one speaker, with start/end timestamps in MM:SS format (relative to the start of THIS audio clip — 00:00 is where this clip begins, not the wider episode) and the spoken text.
...
export const MEDIA_DIARIZATION_PROMPT = `You are transcribing and diarizing a two-or-more-person conversational podcast episode.
...
${MEDIA_DIARIZATION_SHARED_RULES}
 ...
 * MEDIA.CHUNK_DURATION_SECONDS). The request's audio parts are, in
