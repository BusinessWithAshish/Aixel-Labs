...
import {
  MEDIA_DIARIZATION_CONTINUATION_PROMPT_HEADER,
...
async function diarizeChunkContinuation(
  ...
  const knownIds = [...referenceClips.keys()].slice(0, MEDIA.MAX_REFERENCE_SPEAKERS);
  ...
  const prompt = MEDIA_DIARIZATION_CONTINUATION_PROMPT_HEADER.replace(
    "{{REFERENCE_LIST}}",
    referenceList,
...
/**
 * Extracts a short reference clip (MEDIA.REFERENCE_CLIP_SECONDS) for each
 ...
 * MEDIA.MAX_REFERENCE_SPEAKERS total across the whole episode.
 */
async function ensureReferenceClips(
  ...
  for (const speaker of speakers) {
    ...
    if (referenceClips.size >= MEDIA.MAX_REFERENCE_SPEAKERS) continue;
    ...
    const clipDur = Math.min(MEDIA.REFERENCE_CLIP_SECONDS, segDur);
...
/**
 * Splits `sourcePath` into sequential MEDIA.CHUNK_DURATION_SECONDS audio
 ...
 * confirmed not tunable away, see MEDIA.CHUNK_DURATION_SECONDS in
 ...
 * This is best-effort, not guaranteed — see video/README.md.
 */
async function diarizeChunked(
  ...
  for (let t = 0; t < totalDurationSeconds; t += MEDIA.CHUNK_DURATION_SECONDS) {
    chunkStarts.push(t);
  }
...
export async function diarizeFromSource(
  ...
  try {
    ...
    if (durationSeconds <= MEDIA.CHUNK_THRESHOLD_SECONDS) {
