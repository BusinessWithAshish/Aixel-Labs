...
import { MEDIA_TRANSCRIBE, MEDIA_TRANSCRIBE_ERROR_MESSAGES } from "./constants";
...
export async function normalizeToFlac(inputPath: string): Promise<NormalizedAudio> {
  if (!ffmpegPath) {
    throw new Error(MEDIA_TRANSCRIBE_ERROR_MESSAGES.FFMPEG_FAILED);
  }
  ...
  try {
    await execFileAsync(ffmpegPath, [
      ...
      "-vn",
      "-ac",
      String(MEDIA_TRANSCRIBE.TARGET_CHANNELS),
      "-ar",
      String(MEDIA_TRANSCRIBE.TARGET_SAMPLE_RATE_HZ),
      "-c:a",
      "flac",
      outputPath,
    ]);
    ...
    throw new Error(`${MEDIA_TRANSCRIBE_ERROR_MESSAGES.FFMPEG_FAILED}: ${message}`);
  }
