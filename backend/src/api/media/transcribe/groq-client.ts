...
export type GroqTranscribeOptions = {
   ...
   * this with a disfluency-heavy sample. See `MEDIA_CONDENSE.VERBATIM_PROMPT`.
   */
  prompt?: string;
};
...
export async function transcribeWithGroq(
  ...
  if (!apiKey) {
    throw new Error(MEDIA_TRANSCRIBE_ERROR_MESSAGES.MISSING_API_KEY);
  }
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `${MEDIA_TRANSCRIBE_ERROR_MESSAGES.GROQ_FAILED}: ${res.status} ${body}`,
    );
  }
