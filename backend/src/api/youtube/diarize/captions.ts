import { YOUTUBE_DEFAULT_COUNTRY } from "../constants";
import { parseYoutubeVideoId } from "../helpers";
import { YOUTUBE_TRANSCRIPT_DEFAULT_HL } from "../transcript/constants";
...
export function youtubeCaptionsToDiarizedTranscript(
  ...
  if (lines.length === 0) {
    throw new Error(`${YOUTUBE_DIARIZE_ERROR_MESSAGES.CAPTIONS_EMPTY}: no caption lines`);
  }
  ...
  if (utterances.length === 0) {
    throw new Error(`${YOUTUBE_DIARIZE_ERROR_MESSAGES.CAPTIONS_EMPTY}: no speech in captions`);
  }
...
export async function diarizeFromYoutubeCaptions(
  ...
  if (!videoId) {
    throw new Error(
      `${YOUTUBE_DIARIZE_ERROR_MESSAGES.YOUTUBE_METADATA_FETCH_FAILED}: invalid YouTube URL`,
    );
  }
