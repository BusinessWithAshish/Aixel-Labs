...
export const CLIP_DURATION_BOUNDS_FIELDS = {
  minClipSeconds: z
    .number()
    .int()
    .min(MEDIA.MIN_CLIP_SECONDS)
    .max(MEDIA.MAX_CLIP_SECONDS)
    .optional()
    .default(MEDIA.DEFAULT_MIN_CLIP_SECONDS)
    .describe(MEDIA_FIELD_DESCRIPTIONS.minClipSeconds),
  maxClipSeconds: z
    .number()
    .int()
    .min(MEDIA.MIN_CLIP_SECONDS)
    .max(MEDIA.MAX_CLIP_SECONDS)
    .optional()
    .default(MEDIA.DEFAULT_MAX_CLIP_SECONDS)
    .describe(MEDIA_FIELD_DESCRIPTIONS.maxClipSeconds),
  channelContext: z
    .string()
    .max(2000)
    .optional()
    .describe(MEDIA_FIELD_DESCRIPTIONS.channelContext),
  audienceSignals: z
    .array(z.string())
    .max(50)
    .optional()
    .describe(MEDIA_FIELD_DESCRIPTIONS.audienceSignals),
};
...
export const MOMENTS_REQUEST_SCHEMA = z
  .object({
 ...
 */
export const MOMENTS_RESPONSE_VALIDATOR = z.object({
  candidates: z.array(
    z.object({
      rank: z.number(),
      start: z.string(),
