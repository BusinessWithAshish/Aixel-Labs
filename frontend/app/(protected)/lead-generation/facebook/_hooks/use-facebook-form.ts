...
import { FACEBOOK_REQUEST_SCHEMA } from '@aixellabs/backend/facebook/schemas';
...
import { DEFAULT_FACEBOOK_FORM_VALUES } from '../_constants';

export const useFacebookForm = () => {
    const { submitLeadGenScraperForm } = useLeadGenScraper(LEAD_GENERATION_SUB_MODULES.FACEBOOK);

    const form = useForm<FACEBOOK_REQUEST>({
        resolver: zodResolver(FACEBOOK_REQUEST_SCHEMA) as Resolver<FACEBOOK_REQUEST>,
        defaultValues: DEFAULT_FACEBOOK_FORM_VALUES,
    });

    const onSubmit = async (data: FACEBOOK_REQUEST) => {
        await submitLeadGenScraperForm({
            body: data,
            onSuccess: () => {
                form.reset(DEFAULT_FACEBOOK_FORM_VALUES);
            },
        });
    };
...
export type UseFacebookFormReturn = ReturnType<typeof useFacebookForm>;
