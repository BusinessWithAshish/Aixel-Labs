...
| `AIXEL_VPS` | Explicit opt-in, nothing sets it automatically — VPS systemd sets `AIXEL_VPS=1`. Gates the `chatgpt` module (headful Chrome/CDP — needs the VPS's Xvfb + logged-in profile, can't run anywhere else); refused everywhere else, local dev included (`IS_VPS_RUNTIME` in `config.ts`). **Not** used by `claude` — that module just shells to the `claude` CLI, which runs on any machine it's installed and authenticated on, and fails with a plain error otherwise. |
