...
/**
 * Deletes a user lead by leadId for a specific user.
 */
export const deleteUserLead = async (
  ...
  const result = await userLeadsCollection.deleteOne({ userId, leadId });
  return result.deletedCount > 0;
};

/**
 * Deletes multiple user leads by source for a specific user.
 */
export const deleteUserLeadsBySource = async (
  userId: ObjectId,
  source?: LeadSource,
  dbName?: string
): Promise<number> => {
  ...
  // Build query to find leads by source
  const query: any = { _id: { $in: leadIds } };
  if (source) {
    query.source = source;
  }
  ...
  // Delete user leads
  const result = await userLeadsCollection.deleteMany({
    userId,
...
/**
 * Updates notes for a user lead.
 */
export const updateUserLeadNotes = async (
  userId: ObjectId,
  leadId: ObjectId,
  notes: string,
  dbName?: string
...
/**
 * Updates notes for multiple user leads.
 */
export const updateUserLeadsNotes = async (
  userId: ObjectId,
  leadIds: ObjectId[],
  notes: string,
  dbName?: string
  ...
  const result = await userLeadsCollection.updateMany(
    { userId, leadId: { $in: leadIds } },
    { $set: { notes, updatedAt: new Date() } }
  );
